...
# Function to tear down the demo
tear_down() {
    print_step "Tearing down Hello Workflow demo..."
    ...
    # Remove the hello_workflow directory
    if [ -d "$TARGET_DIR" ]; then
        print_step "Removing $TARGET_DIR directory..."
        rm -rf "$TARGET_DIR"
        print_success "Directory $TARGET_DIR removed"
    else
        print_warning "Directory $TARGET_DIR does not exist"
    fi
    
    print_success "🧹 Teardown complete! All demo files removed."
}
...
show_completion() {
    ...
    echo -e "  🔹 To remove demo: ${CYAN}./demo_hello_workflow.sh --teardown${NC}"
    echo
    print_message $CYAN "================================================"
    echo
}
...
show_help() {
    echo "🚀 Hello Workflow Demo Script"
    echo
    echo "Usage: $0 [OPTION]"
    echo
    echo "Options:"
    echo "  --start      Start the Hello Workflow demo (default)"
    echo "  --teardown   Remove the hello_workflow directory and stop services"
    echo "  --help       Show this help message"
    echo
    echo "Examples:"
    echo "  $0                    # Start the demo"
    echo "  $0 --start            # Start the demo"
    echo "  $0 --teardown         # Remove demo and clean up"
    echo "  $0 --help             # Show help"
    echo
    echo "The demo creates a complete standalone workflow project in ./hello_workflow/"
    echo "with FastAPI backend, React frontend, and interactive web interface."
    echo
}
